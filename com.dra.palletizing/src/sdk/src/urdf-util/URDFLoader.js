...
class URDFLoader {
  ...
  load(urdf, onComplete, onProgress, onError) {
    ...
    loadXhr.onprogress = (res)=>{
      if (res.target.statusText === "OK") {
        if (onProgress) {
          onProgress(null);
        }

      } else {
        throw new Error(`URDFLoader: Failed to load url '${ urdfPath }' 
          with error code ${ res.status } : ${ res.statusText }.`);
      }
    }
    loadXhr.onload = (data)=>{
      if (this.workingPath === '') {
      ...
      const model = this.parse(decode.decode(data.target.response));
      onComplete(model);
